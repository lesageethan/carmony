...
class OBDHandler():
    ...
    def get_guitar_volume(self):
        ...
        if guitar_percent < 0.3:
            return 0
        return float(((guitar_percent-0.30)**0.2).real)
    
    def get_drums_2_volume(self):
        drums_percent = self.speed/70
        if drums_percent < 0.4:
            return 0
        return float(((drums_percent-0.40)**0.2).real)
    
    def get_volumes(self):
        volumes = [
            self.get_bass_volume(),
            self.get_pad_volume(),
            self.get_synth_volume(),
            self.get_guitar_volume(),
